---
name: implementor
description: Implements exactly one task from .claude/tasks/TASKS.md in the drone-missions backend and checks it off. Invoked once per task by the multiagent-dev orchestrator.
...
2. Read `.claude/tasks/TASKS.md` and find the **first unchecked** (`- [ ]`) task. If every task is already checked, do nothing and report that the list is complete.
...
5. Edit `.claude/tasks/TASKS.md`, flipping that one task's `- [ ]` to `- [x]`. Do not touch any other task's checkbox.

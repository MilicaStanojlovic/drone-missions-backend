...
#
# Usage: bash scripts/multiagent-dev.sh [--replan] [--dry-run] [--feature <slug>] [--repo <path>] [--max-iterations N] [--add-dir <path>] <feature brief...>
...
#   --dry-run prints the resolved repo/slug/plan-file and the resume decision, then exits without invoking claude.
...
while [ "$(unchecked)" -gt 0 ]; do
  ...
  before=$(unchecked)
  run_stage "Implement $i ($before task(s) left)" "Use the implementor agent to implement the first unchecked task in $PLAN_FILE and check it off in that file."
